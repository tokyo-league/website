...
const sections = [
  {
    ...
    steps: [
      ...
      "`Admin` テーブルに同じメールアドレスが登録されている場合だけ管理画面へ入れます。未登録メールはログイン不可です。",
  ...
  },
  {
    ...
    callouts: [
      "① 担当者を追加・更新します。同じメールアドレスで保存すると表示名・ロールが更新されます。",
      "② Editorに担当リーグを割り当てます。",
      "③ 割当一覧です。「解除」で担当リーグを外します。",
      "④ 登録済み担当者です。削除可能なEditorのみ削除できます。",
      "⑤ OwnerとEditorの権限概要です。",
    ],
    steps: [
      "「担当者を追加」でGoogleメールアドレス、表示名、ロールを入力し「担当者を保存」を押します。",
      "担当者情報を更新したい場合は、同じメールアドレスで表示名またはロールを変更して保存します。",
  ...
  },
  {
    ...
    notes: [
      ...
      "現行UIでは順位表行の個別削除ボタンは表示されません。所属チーム構成や順位表の修正は、まとめて保存で上書きします。",
    ],
  },
  {
    title: "8. お知らせ管理: 一覧・新規作成・更新・削除",
    image: "news-list.png",
    markers: [
      ["①", 84, 25],
      ["②", 80, 43],
...
const globalChecklist = [
  ["ログイン", "Google認証、管理者メール登録、ログアウト、未登録メール不可"],
  ["権限", "Owner / Editor、担当リーグ割当、割当解除、担当者削除制限"],
  ["大会", "年度追加、大会追加、リーグ追加、所属チーム追加・解除"],
  ["試合", "対象リーグ選択、結果画像追加・差替、試合追加・更新・削除"],
  ["順位表", "まとめて保存、試合結果から再計算、登録内容確認"],
  ...
  <section class="page limit-page">
    ...
    <table>
      <tbody>
        ...
        <tr><th>順位表行</th><td>登録済み順位表はまとめて保存で上書きします。個別行削除ボタンは現行UIでは表示されません。行削除・行追加・入力クリアを追加実装候補とします。</td></tr>
        ...
        <tr><th>チーム項目</th><td>Instagram URL はデータ項目として存在しますが、現行フォームには入力欄がありません。必要に応じて追加します。</td></tr>
