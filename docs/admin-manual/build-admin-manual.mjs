  ...
  <section class="page limit-page">
    ...
    <table>
      <tbody>
        ...
        <tr><th>担当者の無効化</th><td>担当者の表示名・管理画面権限・問い合わせ受信設定の変更、無効化、削除に対応済みです。無効化すると管理画面ログインと、設定済みの問い合わせメール配信を停止します。</td></tr>
        <tr><th>問い合わせ受信先</th><td>公開サイトの問い合わせフォームは、有効かつ「問い合わせ先としてメールを受信する」が有効な担当者のメールアドレスへ配信します。Owner / Editorと重複して設定でき、受信だけを止める場合はチェックを外して更新します。管理画面にログインさせない受信専用担当者には「問い合わせ専用（管理画面にはログインできません）」を設定します。</td></tr>
        ...
        <tr><th>トップページ</th><td>メインメッセージ、紹介見出し、サブメッセージの編集に対応済みです。メインメッセージはサイト共通フッターにも同じ内容で反映されます。文言の新規追加・削除は対象外です。</td></tr>
        <tr><th>東京リーグについて</th><td>組織概要、主な事業、東京少年サッカー連盟の根本原則、努力目標の編集に対応済みです。主な事業と努力目標は1行ごとに公開ページの箇条書きとして反映されます。</td></tr>
        ...
        <tr><th>固定ページ・問い合わせ本文設定・会場</th><td>トップページの3つのメッセージと「東京リーグについて」の主要本文は管理画面で編集できます。問い合わせ受信先は担当割当画面で管理できます。問い合わせページ本文、会場マスタの管理UIはなく、追加実装候補です。</td></tr>
