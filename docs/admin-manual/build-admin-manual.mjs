...
const sections = [
  ...
  },
  {
    ...
    callouts: [
      "① メールアドレス、表示名、管理画面権限を入力し、必要に応じて「問い合わせ先としてメールを受信する」を選びます。同じメールアドレスで保存すると既存担当者を更新できます。",
      "② Editorに担当リーグを割り当てます。",
      "③ 表示名を入力します。",
      "④ 管理画面権限を選びます。",
    ...
    steps: [
      "「担当者を追加」でメールアドレス、表示名、管理画面権限を入力します。問い合わせを受信させる場合は「問い合わせ先としてメールを受信する」にチェックを入れ、「担当者を保存」を押します。",
    ...
    notes: [
      ...
      "問い合わせ受信: 「問い合わせ先としてメールを受信する」を有効にした担当者へ、公開フォームから送信された問い合わせメールを配信します。Owner／Editorと重複して設定できます。",
