...
- Ownerのみ編集可能
- 既存文言の編集専用とし、新規追加・削除操作は設けない
- メインメッセージはトップページのヒーローとサイト共通フッターで同じ値を表示する
- 保存時はトップページと共通レイアウトを再検証し、公開ページのフッターにも反映する
...
- ニュース、トップページ、固定ページ、資料のようなリーグ非依存コンテンツは、当面 `owner` または全体編集担当のみ更新可能にする
...
- 納品前に `npm run docs:admin:qa:check` を実行し、管理者説明書QA画像20ページの寸法、非白紙率、文字/濃色ピクセル、色数を確認する
