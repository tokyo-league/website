...
const pages = [
  {
    ...
    lead: "通常の「結果管理」は個別確認・編集の画面です。Excelで一括反映する場合は、まず「入稿ウィザードを開く」を押し、専用画面で対象リーグを選択します。",
    simpleSteps: [
      "① 結果管理の「Excelで一括入稿する」から「入稿ウィザードを開く」を押します。",
      "②「年度」で、結果を登録する年度を選択します。",
      "③「大会」と「リーグ」を選択します。",
      "④「現在編集中のリーグ」が手元の結果管理表と一致することを確認します。",
    ],
    remember: [
  ...
  },
  {
    ...
    lead: "入稿ウィザードでは、Excel選択から結果画像登録までの7段階が表示されます。処理完了後は進行状況へ戻り、次の番号と赤いアニメーション枠が次に押すボタンを示します。",
    simpleSteps: [
      "進行状況の番号を押すと、該当する操作まで画面が移動します。",
      "赤いアニメーション枠と「次はここをクリック」が付いたボタンだけを押します。",
      "Excelを選択すると「Excelの内容を読み取る」へ自動でフォーカスが移ります。",
      "途中経過の案内は現在の次の操作だけを1件表示します。前の案内は完了と同時に消えます。",
      "未消化試合の勝ち点補正は任意操作です。通常の再計算とは別のため、必要な場合だけ使用します。",
      "Excelがない場合の手入力・画像差し替えは、通常の結果管理画面で行います。",
    ],
    remember: [
      "赤い強調がないボタンは、現在の入稿手順では押す必要がありません。",
      "番号を押しても強調が見えない場合は、進行状況の次に未完了の番号を押してください。",
    ],
  },
  {
    ...
    simpleSteps: [
      "①「Excelを選択」を押し、「管理表」シートが入った .xlsx / .xls ファイルを選択します。",
      "②「Excelの内容を読み取る」を押します。",
      "読み取り結果の確認画面で、チーム名・試合日・得点・会場・新規／更新の区分を確認します。",
      "チーム名の不一致がある場合は「不一致チームを名寄せワークスペースで確認する」を押します。",
      "名寄せワークスペースでは、最長の近似チーム名が正式名称として自動提案されます。候補がない例外だけ選択します。",
      "一括名寄せの完了ポップアップで「入稿ウィザードに戻ってExcelを再読込する」を押します。",
      "再読込後、エラーが解消されたことを確認してから、試合結果へ反映します。",
    ],
    remember: [
      "対応形式は .xlsx / .xls、ファイルサイズは5MB以下です。得点が未入力の試合はエラーにせず、入稿対象から除外されます。",
      "既存の同一対戦カードは更新され、新しい対戦カードは追加されます。Excelにない既存試合は残ります。",
      "名寄せは略称追加、正式名称側の空欄への情報補完、公開設定、対象リーグへの所属追加をまとめて行います。既存の正式名称側の情報は上書きしません。",
      "Excelに存在しない既存試合は、自動削除されません。",
    ],
  },
  {
    title: "4. 通常の結果管理で個別編集する",
    ...
    lead: "通常の結果管理では、結果画像の直接差し替えや、Excelを使わない試合の個別追加・更新を行います。Excel入稿の通常フローは入稿ウィザードを使用してください。",
    simpleSteps: [
      "① 結果画像を直接差し替える場合は、画像を選択して「結果画像を保存」を押します。",
      "② Excelがない場合は、試合日、ホーム・アウェイ、得点、会場を入力します。",
      "③「試合結果を追加」を押します。",
      "④ 登録済み試合はコンパクトな一覧から必要な試合だけを開いて編集します。",
    ],
    remember: [
  ...
  },
  {
    ...
    simpleSteps: [
      "① 入稿ウィザードで赤く強調された「試合結果から再計算」を押します。未消化試合の補正は必要な場合だけ使用します。",
      "② 順位表にチームを追加する場合は、追加するチームを選択します。",
      "③「行を追加」を押し、順位表の入力行を追加します。",
      "④ 順位・試合数・勝敗・得失点・勝点を確認し、必要に応じて修正します。",
      "内容を確認したら、赤く強調された「順位表をまとめて保存」を押します。",
      "最後に、星取表を開いて確認し「この星取表を結果画像として登録」を押します。",
      "7段階すべて完了すると完了ポップアップが出ます。「公開ページを別タブで開く」で、入稿したリーグの公開結果を確認します。",
    ],
    remember: [
