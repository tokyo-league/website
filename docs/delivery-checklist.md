...
| 納品前証跡レポート | `docs/output/delivery-evidence-*.md` | `npm run delivery:evidence -- --final --production-url https://<production-domain> --production-env-file .env.production.local --include-build --include-e2e` | 実装済み |
...
- clean worktreeで、本番デプロイURLに対して `npm run delivery:evidence -- --final --production-url https://<production-domain> --production-env-file .env.production.local --include-build --include-e2e` を実行し、証跡レポートを保存する
