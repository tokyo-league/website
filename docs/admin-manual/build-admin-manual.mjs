...
const sections = [
  ...
  },
  {
    ...
    callouts: [
      "① 結果管理は個別確認・編集の画面です。",
      "② Excel一括入稿は「入稿ウィザードを開く」から専用画面へ進みます。",
      "③ ウィザードは対象選択から結果画像登録までの進行状況を表示します。",
      "④ 次の番号を押すと、押すべきボタンが赤いアニメーション枠で強調されます。",
    ],
    steps: [
      "Excelを使う場合は「入稿ウィザードを開く」を押します。",
      "ウィザード内で年度・大会・リーグを選び、「現在編集中のリーグ」が正しいことを確認します。",
      "赤いアニメーション枠と「次はここをクリック」が付くボタンだけを、順に押します。",
      "Excelを選択した直後は「Excelの内容を読み取る」へ自動でフォーカスが移ります。",
      "途中案内は現在の次の操作だけが表示され、完了済みの案内は消えます。",
      "Excelを使わない個別追加・画像差し替えは、通常の結果管理画面で行います。",
    ],
    notes: [
      "未消化試合の勝ち点補正は任意操作です。通常の再計算ボタンと混同しないでください。",
      "Excel入稿と手入力は同じ登録済み試合を更新します。",
      "過去大会は結果画像を正本として扱います。",
    ],
  },
  {
    ...
    callouts: [
      ...
      "③ 「管理表」シート入りの .xlsx / .xls を選択します。",
      "④ ファイルを読み取り、内容確認へ進みます。",
    ],
    steps: [
      "「入稿ウィザードを開く」から年度・大会・リーグを選びます。",
      "「Excelファイルを選択」から「管理表」シートが入った .xlsx / .xls を選びます。",
      "「Excelの内容を読み取る」を押します。",
      "読み取り後、試合数、チーム名、試合日、得点、会場、新規／更新の区分を確認します。",
      "チーム名の不一致がある場合は「名寄せワークスペース」を開き、最長の近似チーム名として自動提案された正式名称を確認します。候補がない例外だけ選択します。",
      "名寄せの完了ポップアップで「入稿ウィザードに戻ってExcelを再読込する」を押します。",
      "問題がなければ「試合を反映する」を押し、次に赤く強調された「試合結果から再計算」を押します。順位表の保存と星取表の結果画像登録まで進めます。",
    ],
    notes: [
      "対応形式は .xlsx / .xls、ファイルサイズは5MB以下です。読み取り対象シート名は「管理表」です。得点が未入力の試合はエラーにせず、入稿対象から除外されます。",
      "同じ対戦カードは既存試合を更新し、新しい対戦は追加します。Excelにない既存試合は削除されず残ります。",
      "名寄せでは略称追加、正式名称側の空欄への情報補完、公開設定、対象リーグへの所属追加を一括で行います。正式名称側の入力済み情報は上書きしません。",
      "7段階すべて完了すると、公開ページ確認の完了ポップアップが出ます。「公開ページを別タブで開く」で入稿したリーグの公開結果を確認します。",
  ...
  },
  {
    ...
    steps: [
      "新規追加は「新規追加」を押します。",
      "一覧の「状態」で、公開・下書き・非公開を確認します。状態フィルターで対象だけを絞り込めます。",
  ...
  },
  {
    ...
    steps: [
      ...
      "ホームとアウェイのユニフォームの色・デザインを文字で入力します。",
      "状態を「下書き」「公開」「非公開」から選びます。",
    ...
    notes: [
      ...
      "ユニフォーム情報はホームとアウェイをそれぞれ文字で入力します。未入力のまま保存することもできます。",
      "チーム一覧はチーム名のあいうえお順で表示されます。",
